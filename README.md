...
# 03.벡터 스토어
- 벡터스토어 : 벡터화된 데이터를 효율적으로 저장하고 검색할 수 있도록 설계된 데이터 관리 시스템
  데이터 -> 벡터 변환 -> 유사도 계산 -> 검색/분석
  ex. 크로마(Chroma):온프레미스환경, 파인콘(Pinecone):클라우드환경, 쿼드런트(Qdrant):온프레미스/클라우드환경
- BERT는 문장에서 앞뒤 문맥을 동시에 고려해서 더 깊은 의미를 파악
- 파인콘 : 실시간 검색 기능 장점